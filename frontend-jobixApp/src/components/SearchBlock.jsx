...
export const SearchBlock = ({products}) => {
    ...
    const handleSearch = async() => {
        ...
        try{
            ...
            const data = await res.json();// correccion- hice llamada al back para obtener los products pero no lo estaba llamando en ninguna parte, pero ya lo llamé en la línea 49

            // Filtramos por nombre 
            const lowerQuery = query.toLowerCase();
            const results = data.filter(p => //estaba llamando a "products" pero no a "data", filtraba localmente y no filtraba por disponibilidad
